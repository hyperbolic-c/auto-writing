#!/usr/bin/env python3
"""
AI-powered slide image generation using Nano Banana Pro or MiniMax.
...
Supports:
- Nano Banana Pro (default): Uses smart iterative refinement with quality review
- MiniMax: Direct image generation (no iterative refinement)
...
Usage (Nano Banana Pro):
    python generate_slide_image_ai.py "Title: Introduction to ML" -o slide_01.png

Usage (MiniMax):
    python generate_slide_image_ai.py "Title: Introduction to ML" -o slide_01.png --provider minimax
"""
...
"""
    ...
    def generate_image(self, prompt: str, attachments: Optional[List[str]] = None) -> Optional[bytes]:
        ...
        Args:
            ...
            attachments: Optional list of image file paths to attach as context (OpenRouter only)
...
Generate a high-quality {'visual/figure' if visual_only else 'presentation slide'} that meets all the guidelines above."""
        ...
        if self.provider == "minimax":
            ...
            image_data = self.generate_image(current_prompt, attachments=None)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate presentation slides or visuals using Nano Banana Pro or MiniMax AI",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples (Nano Banana Pro - default):
  python generate_slide_image_ai.py "Title: ML Basics" -o slide_01.png
  python generate_slide_image_ai.py "Neural network diagram" -o figure.png --visual-only

Examples (MiniMax):
  python generate_slide_image_ai.py "Title: ML Basics" -o slide_01.png --provider minimax
  python generate_slide_image_ai.py "Neural network diagram" -o figure.png --provider minimax --aspect-ratio 16:9
...
Environment (MiniMax):
  ...
  IMAGE_PROVIDER        "openrouter" or "minimax"
    ...
    parser.add_argument("--iterations", type=int, default=2,
                       help="Maximum refinement iterations (default: 2, Nano Banana Pro only)")
    ...
    # Validate attachments exist (OpenRouter only)
    if args.attachments and provider != "minimax":
    ...
    try:
        generator = SlideImageGenerator(api_key=api_key, verbose=args.verbose, provider=provider)
